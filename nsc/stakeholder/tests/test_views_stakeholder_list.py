...
def test_list_view(erm_user, client):
    ...
    response = client.get(stakeholder_list_url, user=erm_user)
...
def test_list_view_query_count(
    erm_user, num_stakeholders, client, django_assert_num_queries
):
    """
    ...
    with django_assert_num_queries(9):  # 5 for view, 4 for login.
        client.get(stakeholder_list_url, user=erm_user)
...
def test_search_form_blank(erm_user, client):
    """
    Test that the fields in the search form are initially blank.
    """
    form = client.get(stakeholder_list_url, user=erm_user).forms[1]
    assert form["name"].value == ""
    assert form["condition"].value == ""


def test_search_on_stakeholder_name(erm_user, client):
    """
...
def test_search_on_stakeholder_country(erm_user, client):
    """
...
def test_search_form_shows_name_term(erm_user, client):
    """
...
def test_search_form_shows_condition_term(erm_user, client):
    """
