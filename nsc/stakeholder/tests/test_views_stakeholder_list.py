...
def test_list_view(django_app):
    ...
    response = django_app.get(stakeholder_list_url)
...
def test_list_view_query_count(num_stakeholders, django_app, django_assert_num_queries):
    """
    Test that fetching the list takes a fixed number of queries.
    """
    baker.make(Stakeholder, _quantity=num_stakeholders)
    with django_assert_num_queries(3):
        django_app.get(stakeholder_list_url)
...
def test_search_form_blank(django_app):
    ...
    """
    form = django_app.get(stakeholder_list_url).form
    assert form["name"].value == ""
    assert form["condition"].value == ""


def test_search_on_stakeholder_name(django_app_form):
    """
    Test the list of policies can be filtered by the stakeholder name.
    """
    baker.make(Policy, name="name")
    response = django_app_form(stakeholder_list_url, name="other")
...
def test_search_on_condition_name(django_app_form):
    ...
    response = django_app_form(stakeholder_list_url, condition="Other")
    assert not response.context["object_list"]
...
def test_search_form_shows_name_term(django_app_form):
    ...
    """
    form = django_app_form(stakeholder_list_url, name="name").form
...
def test_search_form_shows_condition_term(django_app_form):
    ...
    """
    form = django_app_form(stakeholder_list_url, condition="other").form
    assert form["name"].value == ""
