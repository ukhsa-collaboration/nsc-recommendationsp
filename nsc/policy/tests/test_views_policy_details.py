...
def test_detail_view(erm_user, client):
    ...
    response = client.get(instance.get_admin_url(), user=erm_user)
    assert response.context["policy"] == instance


def test_detail_view__when_archived(erm_user, client):
    """
...
def test_back_link(erm_user, client):
    ...
    detail = client.get(instance.get_admin_url(), user=erm_user)
    results = detail.click(linkid="back-link-id")
    assert results.request.path == reverse("policy:list")


def test_back_link_discards_search_results(erm_user, client):
    """
    ...
    """
    instance = baker.make(Policy, name="condition", ages="{child}")
    form = client.get(reverse("policy:list"), user=erm_user).forms[1]
    form["name"] = "condition"
    results = form.submit()
...
def test_manage_review_button(erm_user, client):
    """
    ...
    response = client.get(policy.get_admin_url(), user=erm_user)
    nodes = BeautifulSoup(response.content, "html.parser")
    link = nodes.find("a", {"id": "manage-review-link-id"})
    assert link.text.strip() == gettext("Manage product")
    assert link["href"] == (reverse("review:detail", kwargs={"slug": review.slug}))


def test_manage_versus_create_button(erm_user, client):
    """
