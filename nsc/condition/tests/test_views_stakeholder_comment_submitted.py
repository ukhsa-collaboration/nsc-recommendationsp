...
def response(review_in_consultation, client):
    ...
    url = reverse(
        "condition:stakeholder-comment-submitted", kwargs={"slug": policy.slug}
    )
...
def test_back_link(response, dom):
    """
    Test the back link returns to the stakeholder comment page.
    """
    policy = response.context["condition"]
    expected = reverse("condition:stakeholder-comment", kwargs={"slug": policy.slug})
